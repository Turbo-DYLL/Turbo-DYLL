...
def run(agent_class, start_line, agent_config_file_path: Path, carla_config_file_path: Path,
        num_laps: int = 10) -> Tuple[float, int, int]:
    """
    Run the agent along the track and produce a score based on certain metrics
    Args:
        start_line: int representing the starting line of the waypoint
    ...
    with open(agent_config.waypoint_file_path) as f:
        waypoints = f.readlines()
        start_waypoint = waypoints[start_line]

    temp_waypoint_path = Path(agent_config.waypoint_file_path).parent / "waypoints.temp"

    with open(temp_waypoint_path, "w") as f:
        f.writelines(waypoints[start_line:])

    agent_config.waypoint_file_path = str(temp_waypoint_path)

    waypoint_transforms = [util.convert_transform_from_str_to_agent(waypoint_str) for waypoint_str in waypoints]
    ...
    try:
        ...
        waypoints.waypoints = waypoint_transforms
        carla_runner.world.player.set_transform(util.convert_transform_from_str_to_source(start_waypoint))
...
def main():
    ...
    # edit start line for starting in different locations
    start_line = 1000
